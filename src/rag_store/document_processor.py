...
class ProcessorRegistry:
    ...
    def get_processor_for_file(self, file_path: Path) -> DocumentProcessor | None:
        ...
        if extension == '.doc':
            try:
                # Check if we should route to Office processor for RTF content
                if FileContentDetector.should_use_rtf_processor(file_path):
                    # Look for Office processor and mark it as handling this .doc file with RTF content
                    office_processor = self._processors.get('OfficeProcessor')
                    if office_processor:
